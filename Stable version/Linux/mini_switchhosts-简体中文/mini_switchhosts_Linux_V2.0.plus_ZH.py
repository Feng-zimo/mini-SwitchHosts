...
def run_as_admin():
    ...
    if not is_admin():
        try:
            ...
            try:
                subprocess.Popen(['gksudo', sys.executable] + sys.argv)
                return False
            except:
                try:
                    subprocess.Popen(['kdesudo', sys.executable] + sys.argv)
                    return False
                except:
                    return False
    return True
...
class HostsManagerThread(QThread):
    ...
    def download_hosts(self):
        ...
        if self.target_type == 'github':
            ...
            sources = [
                ...
                "https://raw.githubusercontent.com/521xueweihan/GitHub520/main/hosts",  # 这个源也包含一些replit规则
        ...
        for i, source in enumerate(sources):
            try:
                ...
                response = requests.get(source, timeout=10)  # 缩短超时时间
                if response.status_code == 200:
                    ...
                    # 验证规则是否有效
                    if rules and not rules.startswith("# 未找到"):
                        self.progress_signal.emit(100)
                        self.result_signal.emit({'success': True, 'rules': rules, 'source': source})
                        return
                    else:
                        self.log_signal.emit("️⚠ 下载的规则为空，尝试下一个源...")
                    
            except requests.exceptions.Timeout:
                self.log_signal.emit(f"⚠️  {source} 连接超时")
                continue
            except requests.exceptions.ConnectionError:
                self.log_signal.emit(f"⚠️  {source} 连接错误")
                continue
            except Exception as e:
                self.log_signal.emit(f"⚠️  {source} 失败: {str(e)}")
                continue

        self.result_signal.emit({'success': False, 'error': '所有源都尝试失败，请检查网络连接'})
    ...
    def apply_hosts(self):
        ...
        self.log_signal.emit("🛡️ 检查root权限...")
        if not is_admin():
            self.result_signal.emit({'success': False, 'error': '需要root权限，请以root身份运行程序'})
            return
        ...
        try:
            ...
            self.result_signal.emit({'success': False, 'error': f'权限拒绝: {str(e)}。请确保以root身份运行程序。'})
        except Exception as e:
...
class HostsManager(QMainWindow):
    ...
    def check_admin_status(self):
        """检查并显示管理员状态"""
        if is_admin():
            self.admin_label.setText("✅ 当前以root权限运行")
            self.admin_label.setStyleSheet("color: green; font-weight: bold;")
        else:
            self.admin_label.setText("⚠️ 当前未以root权限运行（部分功能可能受限）")
    ...
    def apply_rules(self):
        ...
        if not is_admin():
            target_name = "GitHub" if self.current_target == "github" else "Replit"
            reply = QMessageBox.question(self, "需要root权限",
                                         f"应用{target_name}规则需要root权限。是否立即以root身份重新启动程序？",
    ...
    def on_apply_result(self, result):
        ...
        if result['success']:
            ...
            self.log("💡 建议刷新DNS缓存: sudo systemctl restart NetworkManager (Linux)")
            QMessageBox.information(self, "成功",
                                    f"{target_name} 规则应用成功！\n\n建议在终端中运行: sudo systemctl restart NetworkManager 来刷新DNS缓存")
    ...
    def restore_backup(self):
        ...
        if not os.path.exists(backup_dir):
            QMessageBox.information(self, "信息", "未找到备份目录")
            return

        # 检查管理员权限
        if not is_admin():
            reply = QMessageBox.question(self, "需要root权限",
                                         "恢复备份需要root权限。是否立即以root身份重新启动程序？",
                                         QMessageBox.Yes | QMessageBox.No)
            if reply == QMessageBox.Yes:
                run_as_admin()
                sys.exit(0)
        ...
        backup_file, _ = QFileDialog.getOpenFileName(
            self, "选择备份文件", backup_dir, "Backup Files (hosts_backup_*)")
