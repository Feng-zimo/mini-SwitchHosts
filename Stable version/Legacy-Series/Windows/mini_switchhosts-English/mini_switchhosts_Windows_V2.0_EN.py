...
class HostsManager(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.btn_backup.clicked.connect(self.create_backup)  # Connect to correct method
    ...
    def check_admin_status(self):
        ...
        if is_admin():
            self.admin_label.setText("✅ Currently running with administrator privileges")
            self.admin_label.setStyleSheet("color: green; font-weight: bold;")
        else:
            self.admin_label.setText("⚠️ Not running with administrator privileges (some functions may be limited)")
    ...
    def apply_rules(self):
        ...
        if not is_admin():
            target_name = "GitHub" if self.current_target == "github" else "Replit"
            reply = QMessageBox.question(self, "Administrator privileges required",
                                         f"Applying {target_name} rules requires administrator privileges. Restart the program as administrator now?",
    ...
    def on_apply_result(self, result):
        ...
        if result['success']:
            ...
            self.log("💡 Suggested to flush DNS cache: ipconfig /flushdns (Windows)")
            QMessageBox.information(self, "Success",
                                    f"{target_name} rules applied successfully!\n\nSuggested to run in command prompt: ipconfig /flushdns to flush DNS cache")
    ...
    def create_backup(self):
        """Create hosts backup - improved version, allows user to select backup location"""
        try:
            ...
            backup_file, _ = QFileDialog.getSaveFileName(
                self, 
                "Select backup file save location", 
                default_backup_name,
                "All Files (*)"
            )
    ...
    def restore_backup(self):
        ...
        backup_file, _ = QFileDialog.getOpenFileName(
            self, "Select backup file", "", "Backup Files (hosts_backup_*)")
...
def main():
    """Main function"""
    try:
        # Check administrator privileges, request elevation if not administrator
